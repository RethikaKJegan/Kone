...
async function setup() {
  ...
  localStorage.setItem('salesnxt_token', authRes.data.tokens.access.token)
  const projRes = await apiClient.post('/projects', { name: 'Offering Test Project' })
...
describe('offerings API', () => {
  it('creates a new offering', async () => {
    ...
    const res = await apiClient.post(`/projects/${projectId}/offerings`)
  ...
  it('patches offering fields', async () => {
    const projectId = await setup()
    const offeringRes = await apiClient.post(`/projects/${projectId}/offerings`)
    const id = offeringRes.data.id
    const patchRes = await apiClient.patch(`/offerings/${id}`, {
      environments: ['car'],
  ...
  it('runs AI placement and returns deterministic pins', async () => {
    ...
    const offeringRes = await apiClient.post(`/projects/${projectId}/offerings`)
    const id = offeringRes.data.id
    await apiClient.patch(`/offerings/${id}`, { selectedComponents: ['ceiling', 'lci'] })
    const placementRes = await apiClient.post(`/offerings/${id}/ai-placement`)
  ...
  it('completes an offering', async () => {
    ...
    const offeringRes = await apiClient.post(`/projects/${projectId}/offerings`)
    const id = offeringRes.data.id
    const completeRes = await apiClient.post(`/offerings/${id}/complete`)
